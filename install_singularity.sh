...
export VERSION=1.15.6 OS=linux ARCH=amd64 && rm go$VERSION.$OS-$ARCH.tar.gz && wget https://dl.google.com/go/go$VERSION.$OS-$ARCH.tar.gz && sudo tar -C /usr/local -xzvf go$VERSION.$OS-$ARCH.tar.gz && rm go$VERSION.$OS-$ARCH.tar.gz
